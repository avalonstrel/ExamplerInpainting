...
from models.loss import SNDisLoss, SNGenLoss, ReconLoss, L1ReconLoss, MaskDisLoss
...
def validate(nets, loss_terms, opts, dataloader, epoch, devices=(cuda0, cuda1), batch_n="whole"):
    ...
    validate phase
    """
    netG, netD, maskNetD = nets["netG"], nets["netD"], nets["maskNetD"]
    GANLoss, ReconLoss, L1ReconLoss, DLoss, MaskDLoss = loss_terms["GANLoss"], loss_terms["ReconLoss"], loss_terms["L1ReconLoss"], loss_terms["DLoss"], loss_terms["MaskDLoss"]
    optG, optD, optMD = opts["optG"], opts["optD"], opts["optMD"]
    ...
    losses = {"g_loss":AverageMeter(), "r_loss":AverageMeter(), "r_ex_loss":AverageMeter(), "whole_loss":AverageMeter(), 'd_loss':AverageMeter(),
              'mask_g_loss':AverageMeter(),'mask_d_loss':AverageMeter(), 'mask_rec_loss':AverageMeter(),'mask_whole_loss':AverageMeter()}
    ...
    for i, data in enumerate(dataloader):
        ...
        # forward
        coarse_imgs, recon_imgs, _ = netG(imgs, None, masks)
        ...
        mask_g_loss = GANLoss(mask_pred_neg)
        mask_g_loss = mask_g_loss.to(device0)
        r_loss = ReconLoss(imgs, coarse_imgs, recon_imgs, masks)

        #r_ex_loss = L1ReconLoss(img_exs, recon_ex_imgs)

        whole_loss = g_loss + r_loss + mask_g_loss #+ r_ex_loss

        # Update the recorder for losses
        losses['g_loss'].update(g_loss.item(), imgs.size(0))
        losses['mask_g_loss'].update(mask_g_loss.item(), imgs.size(0))
        ...
        mask_d_loss = MaskDLoss(mask_pred_pos*masks, mask_pred_neg*masks)
        mask_rec_loss = L1ReconLoss(mask_pred_neg, masks, masks=masks)
        mask_whole_loss = mask_rec_loss + mask_d_loss
        ...
        if (i+1) < config.STATIC_VIEW_SIZE:
            ...
            logger.info("Validation Epoch {0}, [{1}/{2}]: Batch Time:{batch_time.val:.4f},\t Data Time:{data_time.val:.4f},\t Whole Gen Loss:{whole_loss.val:.4f}\t,"
                        "Recon Loss:{r_loss.val:.4f},\t GAN Loss:{g_loss.val:.4f},\t Mask GAN Loss:{mask_g_loss.val:.4f},"
                        "\t D Loss:{d_loss.val:.4f}, \t Mask D Loss:{mask_d_loss.val:.4f}, \t Mask R Loss:{mask_rec_loss.val:.4f}"
                        ...
                        ,g_loss=losses['g_loss'], d_loss=losses['d_loss'], mask_g_loss=losses['mask_g_loss'], mask_d_loss=losses['mask_d_loss'], mask_rec_loss=losses['mask_rec_loss']))
...
def train(nets, loss_terms, opts, dataloader, epoch, devices=(cuda0, cuda1), val_datas=None):
    ...
    for i, data in enumerate(dataloader):
        ...
        optD.zero_grad(), netD.zero_grad(), netG.zero_grad(), optG.zero_grad(), optMD.zero_grad(), maskNetD.zero_grad()

        imgs, masks = imgs.to(device0), masks.to(device0)
        imgs = (imgs / 127.5 - 1)
        #img_exs = (img_exs / 127.5 - 1)
        # mask is 1 on masked region
        coarse_imgs, recon_imgs, recon_ex_imgs = netG(imgs, None, masks)
        ...
        complete_imgs = complete_imgs.to(device1)
        mask_pred_neg = maskNetD(complete_imgs)
        complete_imgs = complete_imgs.to(device0)
        ...
        if (i+1) % config.SUMMARY_FREQ == 0:
            ...
            info_terms = {'WGLoss':whole_loss.item(), 'ReconLoss':r_loss.item(), "GANLoss":g_loss.item(), "DLoss":d_loss.item(),
                          "MaskDLoss":mask_d_loss.item(),"MaskRecLoss":mask_rec_loss.item(), "MaskGLoss":mask_g_loss.item()}
...
def main():
    ...
    netG = InpaintGCExamplerNet(n_in_channel=5)
    ...
    dis_loss = SNDisLoss()
    mask_d_loss = MaskDisLoss()
    mask_d_loss = mask_d_loss.to(cuda1)
    ...
    optMD = torch.optim.Adam([{'params':maskNetD.parameters()} ], lr=lr / 100, weight_decay=decay)
