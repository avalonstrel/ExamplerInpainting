    ...
    for i, (ori_imgs, ori_masks) in enumerate(dataloader):
        ...
        for s_j, size in enumerate(TRAIN_SIZES):
            ...
            if i+1 < config.STATIC_VIEW_SIZE:
                ...
                info['val/{}whole_imgs/{}'.format(size, i)] = img2photo(torch.cat([ imgs * (1 - masks), upsampled_imgs, recon_imgs, imgs, complete_imgs], dim=3))
    ...
    for i, (ori_imgs, ori_masks) in enumerate(dataloader):
        ...
        for s_j, size in enumerate(TRAIN_SIZES):
            ...
            if (i+1) % config.SUMMARY_FREQ == 0:
                ...
                info = {
                         'train/{}whole_imgs'.format(size):img2photo(torch.cat([ imgs * (1 - masks), upsampled_imgs, recon_imgs, imgs, complete_imgs], dim=3))
                         }
