...
import models.superresolution.model as sr_model
from models.superresolution.option import SRArgs
import models.superresolution.utility as sr_util
...
result_dir = 'test_result_logs/{}'.format(config.MODEL_RESTORE[:config.MODEL_RESTORE.find('/')])
...
def validate(nets, loss_terms, opts, dataloader, epoch, network_type, config, devices=(cuda0,cuda1),  batch_n="whole_test_show"):
    """
    validate phase
    """
    ...
    val_save_dir = os.path.join(result_dir, "val_sr_{}_{}_{}".format(config.DATASET, config.MASKDATASET, config.IMG_SHAPES[0]))
    ...
    val_save_sr_dir = os.path.join(val_save_dir, "sr")
    for size in SIZES_TAGS:
        ...
        if not os.path.exists(os.path.join(val_save_sr_dir, size)):
            os.makedirs(os.path.join(val_save_sr_dir, size))
    info = {}
    t = 0
    for i, (ori_imgs, ori_masks) in enumerate(dataloader):
        ...
        for s_i, size in enumerate(TRAIN_SIZES):
            ...
            sr_imgs = complete_imgs
            for k in range(2-s_i):
                sr_imgs = netSR(sr_imgs, 2)
            ...
            sr_img = Image.fromarray(sr_img[0].astype(np.uint8))
            ...
            sr_img.save(os.path.join(val_save_sr_dir, SIZES_TAGS[s_i], "{}.png".format(i)))
            end = time.time()
...
def main():
    ...
    validate(nets, losses, opts, val_loader,0 , config.NETWORK_TYPE, config, devices=(cuda0,cuda1))
