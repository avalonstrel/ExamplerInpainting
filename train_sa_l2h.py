...
from models.sa_gan_l2h import InpaintSANet, InpaintSADirciminator
...
def validate(netG, netD, GANLoss, ReconLoss, DLoss, optG, optD, dataloader, epoch, device=cuda0, batch_n="whole"):
    ...
    for i, (ori_imgs, ori_masks) in enumerate(dataloader):
        ...
        for size in TRAIN_SIZES:
            ...
            pre_imgs = F.interpolate(pre_complete_imgs, size)

            imgs, masks, pre_imgs = imgs.to(device), masks.to(device), pre_imgs.to(device)
            #masks = (masks > 0).type(torch.FloatTensor)
            ...
            if i+1 < config.STATIC_VIEW_SIZE:
                ...
                info['val/{}whole_imgs/{}'.format(size, i)] = img2photo(torch.cat([ imgs * (1 - masks), coarse_imgs, recon_imgs, imgs, complete_imgs], dim=3))
...
def train(netG, netD, GANLoss, ReconLoss, DLoss, optG, optD, dataloader, epoch, device=cuda0, val_datas=None):
    ...
    for i, (ori_imgs, ori_masks) in enumerate(dataloader):
        ...
        for size in TRAIN_SIZES:
            ...
            pre_complete_imgs = F.interpolate(pre_complete_imgs, size)

            imgs, masks, pre_complete_imgs = imgs.to(device), masks.to(device), pre_complete_imgs.to(device)
            imgs = (imgs / 127.5 - 1)
            ...
            if (i+1) % config.SUMMARY_FREQ == 0:
                ...
                info = {
                         'train/{}whole_imgs'.format(size):img2photo(torch.cat([ imgs * (1 - masks), coarse_imgs, recon_imgs, imgs, complete_imgs], dim=3))
                         }
