...
from .networks import GatedConv2dWithActivation, GatedDeConv2dWithActivation, DeformConv2dWithActivation, DeformDeConv2dWithActivation, SNConvWithActivation, get_pad
...
class InpaintRUNNet(torch.nn.Module):
    ...
    def __init__(self, device, n_in_channel=7, ):
        ...
        self.downsample_layer3 = DeformConv2dWithActivation(4*cnum, 4*cnum, 4, 2, padding=get_pad(64, 4, 2))
        self.eblock3 = nn.Sequential(
            DeformConv2dWithActivation(4*cnum, 8*cnum, 3, 1, padding=get_pad(32, 3, 1)),
            DeformConv2dWithActivation(8*cnum, 8*cnum, 3, 1, padding=get_pad(32, 3, 1)),)

        self.downsample_layer4 = DeformConv2dWithActivation(8*cnum, 8*cnum, 4, 2, padding=get_pad(32, 4, 2))
        self.eblock4 = nn.Sequential(
            DeformConv2dWithActivation(8*cnum, 8*cnum, 3, 1, dilation=2, padding=get_pad(16, 3, 1, 2)),
            DeformConv2dWithActivation(8*cnum, 8*cnum, 3, 1, dilation=4, padding=get_pad(16, 3, 1, 4)),)

        self.dblock4 = nn.Sequential(
            DeformConv2dWithActivation(8*cnum, 16*cnum, 3, 1, dilation=8, padding=get_pad(16, 3, 1, 8)),
            DeformConv2dWithActivation(16*cnum, 16*cnum, 3, 1, dilation=16, padding=get_pad(16, 3, 1, 16)),)

        self.upsample_layer4 = DeformDeConv2dWithActivation(2, 16*cnum, 8*cnum, 3, 1, padding=get_pad(32, 3, 1))
        #concate 8*cnum-> 16*cnum
        self.dblock3 = nn.Sequential(
            DeformConv2dWithActivation(16*cnum, 8*cnum, 3, 1, padding=get_pad(32, 3, 1)),
            DeformConv2dWithActivation(8*cnum, 8*cnum, 3, 1, padding=get_pad(32, 3, 1)),)

        self.upsample_layer3 = DeformDeConv2dWithActivation(2, 8*cnum, 4*cnum, 3, 1, padding=get_pad(64, 3, 1))
        #concate 4*cnum-> 8*cnum
        self.dblock2 = nn.Sequential(
            DeformConv2dWithActivation(8*cnum, 4*cnum, 3, 1, padding=get_pad(64, 3, 1)),
            DeformConv2dWithActivation(4*cnum, 4*cnum, 3, 1, padding=get_pad(64, 3, 1)),)
